...
class ShopLocalDataSource {
  ShopLocalDataSource(this._isarService);
  ...
  Future<ShopProductModel?> getProductById(final String productId) async {
    final isar = await _isarService.db;
    final collection = await isar.shopProductCollections.getByProductId(productId);

    if (collection == null) return null;

    final lastUpdated = collection.lastUpdated;
    if (lastUpdated == null ||
        DateTime.now().difference(lastUpdated) > AppConstants.staleDurationShopProduct) {
